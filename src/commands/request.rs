...
fn prepare_request(path: &str, method: reqwest::Method) -> Result<reqwest::RequestBuilder, Box<dyn std::error::Error>> {
    ...
    let server = current_server.server.as_deref()
        .ok_or("Server URL is missing in the configuration")?;
    let api_key = current_server.api_key.as_deref()
    ...
    Ok(client.request(method, url).header("Authorization", &bearer_token))
}

pub async fn server_request(path: &str, method: reqwest::Method, payload: Option<Value>) -> Result<reqwest::Response, Box<dyn std::error::Error>> {
